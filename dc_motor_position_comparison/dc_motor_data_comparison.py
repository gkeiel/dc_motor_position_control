...
rmse_pid = numpy.sqrt( (e_pid[:N]**2).mean() )
rmse_pr  = numpy.sqrt( (e_pr[:N]**2).mean() )
rmse_rep = numpy.sqrt( (e_rep[:N]**2).mean() )

# plot data
plt.figure(figsize=(10, 6))
plt.plot(t, e_pid[:N], label=f'PID RMSE={rmse_pid:.2f}')
plt.plot(t, e_pr[:N], label=f'PR RMSE={rmse_pr:.2f}')
plt.plot(t, e_rep[:N], label=f'repetitive RMSE={rmse_rep:.2f}')
plt.grid(True)
plt.ylim([-100, 100])
plt.xlim([0, 20])
plt.title('Results with each controller')
plt.xlabel('Time (s)')
plt.ylabel('Signals')
plt.legend()
...
# save figure
plt.tight_layout()
plt.savefig('dc_motor_position_comparison/dc_motor_data_comparison.png', dpi=300)
plt.show()