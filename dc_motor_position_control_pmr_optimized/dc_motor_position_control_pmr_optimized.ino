...
// declare variables
// -----------------
float r, y, e, u, t, t_s, t_a, T, xi, w_0;
uint16_t k_p;
uint16_t k_r;
uint16_t A;
uint8_t k;
...
const uint8_t m = 4;
const uint8_t h[7] = {1, 3, 5, 7, 9, 11, 13};
const uint16_t umax = 1000;
...
void res_coeff(res_mode &h, float omega, float xi, float k_r, float t_a){
  // controller coefficients with pre-warping
  // a_0*u_r(k) = b_0*e(k) +b_1*e(k-1) +b_2*e(k-2) -a_1*u_r(k-1) -a_2*u_r(k-2)
  float alpha = omega/tanf(omega*t_a/2.0);
...
void setup() {
  ...
  // time values
  t   = 0;
  t_s = 0.0001; // switching period (f_s = 10 kHz)
  t_a = t_s*20; // sampling period
  ...
  for(int i = 0; i < m; i++){
    float w_h = h[i]*w_0;
    res_coeff(mode[i], w_h, xi, k_r/3, t_a);
  }
...
void measurement(){
  // read sensor
  float scale_fact = 450.0/128;
  y = (analogRead(A0) -511)*scale_fact;
}
